...
最早的多轮 Agent 方案：每轮读屏幕节点 → 喂给 LLM → LLM 决定下一步操作。在华为上，一旦切到微信，`rootInActiveWindow` 就返回 null 或空节点，LLM 收到空屏幕，开始乱猜联系人名字（"彭丽莎"这个真实联系人名完全读不到，模型自己编了一个）。
...
读到了节点，但 LLM 没有图像，只有文字描述，让它估坐标就是瞎猜。Agent 连续把"菠萝的蜜前端交流群"和"遵义路780弄互助团购群"当成了目标联系人打开。
...
// "跟彭丽莎说我吃过了" → {"intent":"wechat_send","contact":"彭丽莎","message":"我吃过了"}
...
硬编码流程跑起来了，微信搜索框也打开了，键盘也弹出来了，剪贴板也写进去了（键盘上方能看到"彭丽莎"的剪贴板建议）。但就是粘不进去。
...
搞定 `ACTION_PASTE` 的聚焦问题（加 `ACTION_FOCUS` 或长按触发粘贴菜单），然后完整跑通"给彭丽莎发我吃过了"这个流程。
